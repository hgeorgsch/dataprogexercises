...
Utviklingen av nye programmeringssprog har i all hovedsak vært
...
Simula ble aldri noe særlig utbredd som sprog, men det etablerte flere gode
idéer som ble førende for mange sprog som er kommet senere senere.
Det tok dog tyve år før objekt-orientert programmering virkelig slo an.
Lenge var der flere paradigmer som konkurrerte, men de siste tredve årene har
...
Objektet har også metoder, eller funksjoner, som modellerer oppførselen til objektet.
Vi ønsker ikke at andre deler av programmet skal kunne endre verdier i objektet
uten videre, fordi det ofte fører til inkonsistente tilstander.
Når vi skrive metoder, sørger vi for at det gjør all feilsjekking og oppdatering
for å garantere at tilstanden fortsatt er konsistent.
...
Hvis vi skal implementere en teller, for å telle hvor mange ganger en hendelse
intreffer, kan vi se for oss en klasse med to metoder; en for å telle én ny
hendelse og én for å hente ut antall hendelser hittil.
...
Det første er arv.  
Når vi modellerer økosystemet trenger vi både ulver og rever, kaniner og mus,
...
er rovdyr.
Både rovdyr og byttedyr er agenter i vår simulator, 
og de har det til felles at de befinner seg på en plass i landskapet og kan handle.

Når vi programmerer klasser kan vi la en klasse arve en anden hverandre.
...
Dette kaller vi polymorfi, fra gresk, der poly betyr flere og morf betyr form.
...
Grunnen til dette er at python bekjenner seg til det som de kaller *duck typing*.
«Hvis det ser ut som en and, og låter som en and, så er det en and.»

Python bryr seg rett og slett ikke om hvilken type objektet har i det hele tatt.
...
Det er rett og slett en god idé å være ryddig og formell når man modellerer
programvaren sin.
