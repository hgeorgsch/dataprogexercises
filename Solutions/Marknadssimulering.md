...
kr, kurvar = simuler_dag(100, salsvarer)
omsetning = sum(kr)
print("Omsetning med tilbud på epler:", omsetning)

eplepriser = np.arange(10,3,-1)
omsetning_epler_justert = []

for pris in eplepriser:
    salsvarer[0]["salspris"] = pris
    kr, kurvar = simuler_dag(100, salsvarer)
    omsetning_epler_justert.append(sum(kr))

plt.figure(figsize=(10,6))
plt.plot(eplepriser, omsetning_epler_justert)
plt.show()
```
...
```{code-cell} ipython3
def simuler_dag(antall_kunder, varer ):
    penger_liste = []
    handelkurver = []

    for _ in range(antall_kunder):
        budsjett, kundevare = lag_kunde(varer)
        pris, kurv, _ = tilbodshandling(kundevare, budsjett)
        penger_liste.append(pris)
        handelkurver.append(kurv)
    return penger_liste, handelkurver

penger_l, handelkurver = simuler_dag(10, salsvarer)
print(penger_l)
print(handelkurver)
```
