...
```{code-cell} ipython3
print(df["BASE_CUR"].unique()) # viser unike verdier (valutaer) i kolonne "BASE_CUR"

usd = df[ df["BASE_CUR"] == "USD" ]
sek = df[ df["BASE_CUR"] == "SEK" ]
eur = df[ df["BASE_CUR"] == "EUR" ]

currencies = [("USD",usd),( "SEK", sek),("EUR", eur)]

def filter(df, currency):
    df = df.reset_index()
    df[currency] = df["kurs"]
    df = df.filter( items= [ "dato",currency ] )
    return df

new_mrg = mrg
for currency_streng, curr_df in currencies:     # tildeler både dataframe og tilhørende valutastreng til funksjonen filter(). Alternativt kunne man bruke merge_asof direkte mellom new_mrg og hver dataframe.
    new_mrg = pd.merge_asof(new_mrg, filter(curr_df, currency_streng).sort_values("dato"), on = "dato" ) # bruk av sort_values her garanterer at dataframene er kronologisk sortert på dato. 

# For å gjøre grafene mer lesbare har jeg separert skandinaviske valutaer
# og andre valutaer i dataframen i to ulike plott.
new_mrg[["DKK","SEK"]].plot(title="Skandinaviske valutaer")
new_mrg[["USD","GBP", "EUR"]].plot(title="Andre valutaer")
new_mrg
```
